...
import { DEFAULT_LANGUAGE_ID } from '../utils/constants.js'

const env = k =>
  (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env[k]) ||
  (typeof process !== 'undefined' && process.env && process.env[k]) || ''

async function createSubmission(sourceCode, languageId = DEFAULT_LANGUAGE_ID, stdin = '') {
  const url = `${env('VITE_JUDGE0_BASE')}/submissions?base64_encoded=false&wait=true`
  const headers = {
    'Content-Type': 'application/json',
    'X-RapidAPI-Key': env('VITE_JUDGE0_KEY'),
    'X-RapidAPI-Host': 'judge0-ce.p.rapidapi.com'
  }
  const body = JSON.stringify({ source_code: sourceCode, language_id: languageId, stdin })
  const res = await fetch(url, { method: 'POST', headers, body })
  if (!res.ok) throw new Error(`Judge0 error: HTTP ${res.status}`)
  return res.json()
}

async function runTests({ languageId = DEFAULT_LANGUAGE_ID, sourceCode, tests = [] }) {
  const results = []
